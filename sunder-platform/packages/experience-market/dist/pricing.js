import { deriveActivationProfile } from "./activation-profile.js";
import { applyMovementDamageLaneSurcharge } from "./cost-rules.js";
import { resolveModifierData } from "./palette.js";
import { validateAbility } from "./validation.js";
// ── Cost math ─────────────────────────────────────────────────────────────────
export const ZERO_COST = { strings: 0, beats: 0, enhancements: 0 };
...
export function formatCost(cost) {
    const parts = [];
    if (cost.strings)
        parts.push(`${formatSignedNumber(cost.strings)} Strings`);
    if (cost.beats)
        parts.push(`${formatSignedNumber(cost.beats)} Beats`);
    if (cost.enhancements)
        parts.push(`${formatSignedNumber(cost.enhancements)} Enhancements`);
    return parts.length ? parts.join(" · ") : "No cost";
}
function formatUnit(value, singular, plural) {
    return `${value} ${value === 1 ? singular : plural}`;
}
export function formatMarketCost(cost) {
    ...
    if (cost.enhancements !== 0) {
        parts.push(`${cost.enhancements} ${Math.abs(cost.enhancements) === 1 ? "Enhancement" : "Enhancements"}`);
    }
    if (parts.length === 0)
        return "0";
    return `${signPrefix}${parts.join(" · ")}`;
}
export function toneForFamily(family) {
    switch (family) {
...
export function calculateTotalFromCost(cost) {
    // beats are 0.1
    // strings are 1
...
export function computeAbilitySummary(nodes) {
    const root = nodes.find((node) => node.type === 'abilityRoot');
    const profile = deriveActivationProfile(nodes);
    const modifierNodes = nodes.filter((node) => node.type === 'marketModifier');
    ...
    const focus = sumCosts(resolvedModifierData
        .filter((node) => node.lane === 'focus')
        .map((node) => node.cost));
    const flipside = sumCosts(resolvedModifierData
        .filter((node) => node.lane === 'flipside')
        .map((node) => node.cost));
    const body = sumCosts(resolvedModifierData
