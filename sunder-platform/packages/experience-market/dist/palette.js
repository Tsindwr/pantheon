import { POTENTIAL_ABBREVIATIONS } from "@sunderttrpg/core/potentials";
import { RULE_TERM_OPTION_POOLS } from "./rule-terms.js";
// ── Block catalog ─────────────────────────────────────────────────────────────
export const MODIFIER_OPTION_POOLS = {
...
function groupToPaletteTemplate(group) {
    ...
    const selectedOptionId = group.options.some((option) => option.id === group.defaultOptionId)
...
export function getModifierOptionPool(poolId) {
    return MODIFIER_OPTION_POOLS[poolId];
}
function resolveIncreaseRiskinessCost(data, fallback) {
    const isIncreaseRiskinessCaveat = data.optionPoolId === "caveatType" &&
        data.selectedOptionId === "increaseRiskiness";
    if (!isIncreaseRiskinessCaveat)
        return fallback;
...
export function resolveModifierData(data) {
    ...
    if (!pool || pool.options.length === 0)
        return data;
    const option = pool.options.find((candidate) => candidate.id === data.selectedOptionId) ??
        pool.options[0];
    if (!option)
        return data;
...
function resolveOptionLabel(poolId, optionId) {
    if (!optionId)
        return undefined;
    const pool = getModifierOptionPool(poolId);
    return pool?.options.find((option) => option.id === optionId)?.label;
}
function resolvePoolOptionIdOrDefault(poolId, optionId, preferredFallbackOptionId) {
    if (optionId)
        return optionId;
    const pool = getModifierOptionPool(poolId);
    if (preferredFallbackOptionId &&
        pool?.options.some((option) => option.id === preferredFallbackOptionId)) {
        return preferredFallbackOptionId;
    }
    return pool?.options[0]?.id;
}
function resolvePotentialAbbreviation(potentialId) {
    if (!potentialId)
        return undefined;
    return POTENTIAL_ABBREVIATIONS[potentialId];
}
function getModifierCardLabelResolverKey(data) {
    const poolKey = data.optionPoolId;
    const optionKey = data.optionPoolId && data.selectedOptionId
        ? `${data.optionPoolId}:${data.selectedOptionId}`
        : undefined;
    return { poolKey, optionKey };
}
const MODIFIER_CARD_LABEL_OVERRIDES = {
    ...
    damageBase: ({ data, getOptionLabel }) => {
        ...
        const damageDiePotentialId = resolvePoolOptionIdOrDefault("volatilityDieRef", data.selectionValues?.damageDie);
        const damageDieAbbreviation = resolvePotentialAbbreviation(damageDiePotentialId) ?? "P";
        const targetPotentialId = resolvePoolOptionIdOrDefault("potentialRef", data.selectionValues?.targetPotential);
        const targetPotentialLabel = getOptionLabel("potentialRef", targetPotentialId) ?? "Potential";
    ...
    damagePriming: () => "+",
    rangeDistance: ({ selectedOption }) => selectedOption ? `${selectedOption.label}` : "Range",
    movementDistance: ({ selectedOption }) => selectedOption ? `Move ${selectedOption.label}` : "Movement",
    ...
    targetingMode: ({ selectedOption }) => selectedOption ? selectedOption.label : "targeting",
    conditionMinor: ({ data, getOptionLabel }) => {
        const condition = getOptionLabel("minorConditionNameRef", data.selectionValues?.minorConditionName);
        return condition ? `${condition}` : "Minor Condition";
    },
    conditionMajor: ({ data, getOptionLabel }) => {
        const condition = getOptionLabel("majorConditionNameRef", data.selectionValues?.majorConditionName);
    ...
    increase: ({ data, selectedOption, getOptionLabel }) => {
        switch (data.selectedOptionId) {
            case "potential": {
                const potential = getOptionLabel("potentialRef", data.selectionValues?.increasedPotential);
                return potential ? `temp. +1 to ${potential}` : "temp. +1 to Potential";
            }
            case "proficiency": {
                const skill = getOptionLabel("skillRef", data.selectionValues?.increasedSkill);
                return skill ? `temp. ${skill} Proficiency` : "temp. Skill Proficiency";
            }
            case "advantage": {
                const skill = getOptionLabel("skillRef", data.selectionValues?.advantageSkill);
                return skill && skill !== "Other"
                    ? `gain ${skill} Advantage`
                    : "gain Advantage";
            }
            case "domain": {
                const domain = getOptionLabel("domainRef", data.selectionValues?.increasedDomain);
    ...
    recover: ({ data, selectedOption, getOptionLabel }) => {
        switch (data.selectedOptionId) {
            case "stress": {
                const potential = getOptionLabel("potentialRef", data.selectionValues?.recoveredStressPotential);
                return potential ? `recover 1 ${potential} Stress` : "recover Stress";
            }
            case "resistance": {
                const potential = getOptionLabel("potentialRef", data.selectionValues?.recoveredResistancePotential);
                return potential
                    ? `recover 1 ${potential} Resistance`
                    : "recover Resistance";
            }
            case "minor-condition": {
                const condition = getOptionLabel("minorConditionNameRef", data.selectionValues?.recoveredMinorConditionName);
                return condition ? `recover from ${condition}` : "recover Minor Condition";
            }
            case "major-condition": {
                const condition = getOptionLabel("majorConditionNameRef", data.selectionValues?.recoveredMajorConditionName);
                return condition ? `recover from ${condition}` : "recover Major Condition";
            }
            case "stress-track": {
                const potential = getOptionLabel("potentialRef", data.selectionValues?.clearedStressPotential);
                return potential
                    ? `clear ${potential} Stress Track`
                    : "clear Stress Track";
            }
            case "resistance-track": {
                const potential = getOptionLabel("potentialRef", data.selectionValues?.clearedResistancePotential);
                return potential
    ...
    "caveatType:increaseRiskiness": ({ data, getOptionLabel }) => {
        const riskiness = getOptionLabel("riskinessRef", data.selectionValues?.riskinessLevel);
        return riskiness
            ? `${riskiness}`
            : "+1 Riskiness";
    },
    ...
    "caveatType:spendStress": ({ data, getOptionLabel }) => {
        const potentialId = resolvePoolOptionIdOrDefault("potentialRef", data.selectionValues?.spentStressPotential, "might");
        const potential = getOptionLabel("potentialRef", potentialId);
        return potential ? `Spend 1 ${potential} Stress` : "Spend 1 Stress";
    },
    "caveatType:spendResistance": ({ data, getOptionLabel }) => {
        const potentialId = resolvePoolOptionIdOrDefault("potentialRef", data.selectionValues?.spentResistancePotential, "might");
        const potential = getOptionLabel("potentialRef", potentialId);
        return potential ? `spend 1 ${potential} Resistance` : "spend 1 Resistance";
    },
    "caveatType:testRequired": ({ data, getOptionLabel }) => {
        const skillId = resolvePoolOptionIdOrDefault("skillRef", data.selectionValues?.testRequirementSkill, "force");
        const skill = getOptionLabel("skillRef", skillId);
        const potentialSelectionId = data.selectionValues?.testRequirementPotential ?? "default";
        const potential = potentialSelectionId !== "default"
...
export function resolveModifierCardLabel(data) {
    const resolved = resolveModifierData(data);
    const pool = resolved.optionPoolId
        ? getModifierOptionPool(resolved.optionPoolId)
        : undefined;
    const selectedOption = pool?.options.find((option) => option.id === resolved.selectedOptionId) ??
        pool?.options[0];
    const { poolKey, optionKey } = getModifierCardLabelResolverKey(resolved);
    const resolver = (optionKey ? MODIFIER_CARD_LABEL_OVERRIDES[optionKey] : undefined) ??
