import { resolveModifierData } from "./palette.js";
const SPLIT_ACTION_ECONOMIES = new Set([
    "action",
    "twoActions",
    "minute",
    "ritual",
]);
function parseActionEconomyIdFromLabel(label) {
    const normalized = label.trim().toLowerCase();
    if (normalized === "activation · trait")
        return "trait";
    if (normalized === "activation · surge")
        return "surge";
    if (normalized === "activation · action")
        return "action";
    if (normalized === "activation · 2 actions")
        return "twoActions";
    if (normalized === "activation · 1 minute")
        return "minute";
    if (normalized === "activation · ritual")
        return "ritual";
    return "unknown";
}
function parseResetConditionIdFromLabel(label) {
    const normalized = label.trim().toLowerCase();
    if (normalized === "reset · general")
        return "general";
    if (normalized === "reset · spell")
        return "spell";
    if (normalized === "reset · short rest")
        return "shortRest";
    if (normalized === "reset · long rest")
        return "longRest";
    return "unknown";
}
function findActivationTypeNode(nodes) {
    return nodes.find((node) => node.type === "marketModifier" &&
        node.data.optionPoolId === "activationType");
}
function findResetConditionNode(nodes) {
    return nodes.find((node) => node.type === "marketModifier" &&
        node.data.optionPoolId === "resetCondition");
}
function getFallbackActivationNode(nodes) {
    return nodes.find((node) => node.type === "marketModifier" &&
        resolveModifierData(node.data).label.startsWith("Activation ·"));
}
function getFallbackResetNode(nodes) {
    return nodes.find((node) => node.type === "marketModifier" &&
        resolveModifierData(node.data).label.startsWith("Reset ·"));
}
function actionEconomyLabelFromId(actionEconomyId) {
...
function resetLabelFromId(resetConditionId) {
...
export function deriveActivationProfile(nodes) {
    ...
    const focusSide = activationNode?.data.selectionValues?.focusSide === "indirect"
        ? "indirect"
        : "direct";
    return {
        actionEconomyId,
...
export function isActivationProfileModifier(node) {
    const resolved = resolveModifierData(node.data);
    return (node.data.optionPoolId === "activationType" ||
        node.data.optionPoolId === "resetCondition" ||
        resolved.label.startsWith("Activation ·") ||
        resolved.label.startsWith("Reset ·"));
}
//# sourceMappingURL=activation-profile.js.map