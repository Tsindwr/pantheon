import { successLevelAppliesBeat, successLevelAppliesFallout, successLevelAppliesStress } from "../rules/resolution-system.js";
import { convertNumberToSuccessLevel, convertSuccessLevelToNumber } from "../rules/success-levels.js";
import { resolveBaseRoll, resolveBaseRollFromFace } from "./resolve-base-roll.js";
import { resolveVolatilityPoolFromFaces, resolveVolatilityPoolRoll } from "./resolve-volatility-pool.js";
export function buildVolatilityPoolState(state) {
    return {
        domain: !!state.domain,
...
function finalizeResult(input) {
    ...
    const lockedBaseOutcome = naturalCrit ||
        naturalMiff ||
        Boolean(volatilityPoolResult.lockBaseOutcome);
    let finalSuccessLevel = baseRollResult.successLevel;
    let exploded = false;
    if (!lockedBaseOutcome) {
        if (volatilityPoolResult.explode) {
            ...
            finalSuccessLevel = convertNumberToSuccessLevel(convertSuccessLevelToNumber(baseRollResult.successLevel) +
    ...
    if (naturalCrit)
        resistancesRecovered += 1;
    if (Boolean(volatilityPoolResult.recoverResistance))
        resistancesRecovered += 1;
...
export function resolveObservedSunderRoll(state, observed) {
    const baseDie = {
        resistances: state.resistances,
        potentialKey: state.potentialKey,
...
export function resolveSunderRoll(state) {
    const baseDie = {
        resistances: state.resistances,
        potentialKey: state.potentialKey,
        potentialValue: state.potentialValue
...
//# sourceMappingURL=resolve-sunder-roll.js.map