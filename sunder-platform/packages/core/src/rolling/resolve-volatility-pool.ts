...
export function rollDV(dieType: VolatilityDieType): number {
    return Math.floor(Math.random() * dieType + 1);
}

export function resolveVolatilityRoll(
    volatilityState: VolatilityDieState
): VolatilityDieResult {
...
function buildPoolResult(input: {
    ...
    const sorted = [...dieResults].sort((a, b) =>
        plan.keepLowest ? a - b : b - a
    );
    ...
    const defaultModifier = resolveVolatilityModifier(
        effectiveJinxThreshold,
        keptFace
    );
...
export function resolveVolatilityPoolFromFaces(
    ...
    dieResults.forEach((face) =>
        validateVolatilityFace(face, volatilityPoolState.dieType)
    );

    return buildPoolResult({ volatilityPoolState, dieResults, plan });
}

export function resolveVolatilityPoolRoll(
    volatilityPoolState: VolatilityPoolState
): VolatilityPoolResult {
    const plan = getVolatilityPlan(volatilityPoolState);
