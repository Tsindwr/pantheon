...
} from "../rules/resolution-system.js";
import {
    convertNumberToSuccessLevel,
    convertSuccessLevelToNumber
} from "../rules/success-levels.js";
...
} from "./resolve-volatility-pool.js";
import type {
    BaseDieState,
    BaseRollResult,
    ObservedRoll,
...
export function resolveObservedSunderRoll(
    ...
    const volatilityPoolResult = resolveVolatilityPoolFromFaces(
        buildVolatilityPoolState(state),
        observed.volatilityFaces
    );
...
export function resolveSunderRoll(state: TestState): TestResult {
    ...
    const volatilityPoolResult = resolveVolatilityPoolRoll(
        buildVolatilityPoolState(state)
    );
    const final = finalizeResult({ baseRollResult, volatilityPoolResult });
