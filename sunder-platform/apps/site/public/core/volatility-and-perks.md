...
- If a player has [[proficiencies|proficiency]] in the Skill associated with the Test, add a Volatility Die.
- If a player has Volatility in one or more [[proficiencies|Domains]] relevant to this Test, add a Volatility Die.
...
>Taking the Help Action requires a player to declare an intention of how their actions will assist a target with a goal. This allows a player to give the target [[resolution-system#Risky Tests|Advantage]] on that Test, effectively lowering the [[resolution-system|Riskiness]] of that roll.
...
When all Perk slots on a Die are filled, a special Perk that can be assigned to the maximum value is unlocked for that Volatility Die. This special Perk is called a Charge, and is purchasable for a number of [[experience|Beats]] equal to the number of Perk slots on the Volatility Die. When Charged, a Volatility Die explodes when rolling the maximum value on the Volatility Die when its jinx threshold is maxxed out.

Exploding a Volatility Die during a Test causes an automatic [[resolution-system|Crit]], and gives a creature the following benefits:
...
<div class="sunder-premium-slot" data-premium-slot="prm_volatility_perk_catalog"></div>
